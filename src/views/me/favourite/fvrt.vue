      ...
      <div role="tabpanel" aria-hidden="false">
        <div class="all_template">
          <div class="every_template">
            <div class="template_flex_out" v-for="id in favorite_ppt_title.length" :key=id>
              <test-template :imgSrc="favorite_ppt_MiniPicture[id - 1]" :title="favorite_ppt_title[id - 1]"
                :tags="favorite_ppt_tags[id - 1]" :author="favorite_ppt_author[id - 1]" :favr="favorite_ppt_FavrState[id - 1]"
                :downPath="favorite_ppt_path[id - 1]" :turnPath="`#/pptcontent/${favorite_ppt_id[id - 1]}`"
                :pptID="`${favorite_ppt_id[id - 1]}`" @turn_login="turnToLogin"></test-template>
            </div>
          </div>
        </div>
      </div>
...
export default {
  ...
  data: () => {
    return {
      ...
      favorite_ppt_author: [],
      favorite_ppt_id: [],
      favorite_ppt_MiniPicture: [],
      favorite_ppt_FavrState: [],
  ...
  methods: {
    ...
    currentChange(index) {
      this.axios.get(`/user/myFavorite?pageIndex=${index}&pageSize=9`).then((ret) => {
        this.favorite_ppt_title = ret.list.map((v) => { return v.Name; })
        this.favorite_ppt_tags = ret.list.map((v) => { return v.Tags; })
        this.favorite_ppt_path = ret.list.map((v) => { return axiosConfig.staticResourceUrl + v.Path })
        this.favorite_ppt_id = ret.list.map((v) => { return v.ID })
        this.favorite_ppt_MiniPicture = ret.list.map((v) => { return axiosConfig.staticResourceUrl + v.MinPicture })
        this.favorite_ppt_FavrState = ret.list.map((v) => {return v.FavrState})
        this.favorite_ppt_author = ret.list.map((v) => {
  ...
  mounted: function () {
    this.axios.get('/user/myFavorite?pageIndex=1&pageSize=9').then((ret) => {
      this.favorite_ppt_title = ret.list.map((v) => { return v.Name; })
      this.favorite_ppt_tags = ret.list.map((v) => { return v.Tags; })
      this.favorite_ppt_path = ret.list.map((v) => { return axiosConfig.staticResourceUrl + v.Path })
      this.favorite_ppt_id = ret.list.map((v) => { return v.ID })
